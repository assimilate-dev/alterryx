...
will return all resources that you can access via your subscription. However,
only 'apps' can be used with the rest of the API functions. To make sure that
`get_app` only returns apps, use `packageType = "0"` as a request parameter.
